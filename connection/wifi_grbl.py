"""
WiFi GRBL connection backend for TiggyUGS.

Connects to a GRBL controller running on an ESP32 with WiFi via telnet
(TCP port 23). Uses the same text-based GRBL protocol as serial_grbl,
but over a TCP socket instead of a serial port.
"""
...
# Reuse the GRBL parsing regexes and constants from serial_grbl
...
class WiFiGrblConnection(ConnectionBase):
    """Connection to a GRBL controller over WiFi/TCP (telnet port 23)."""
    ...
    def connect_to(self, address: str, **kwargs):
        """Connect to ESP32 GRBL over TCP.
        address: IP string. kwargs: port (default 23)."""
        if self.state != ConnectionState.DISCONNECTED:
            self.disconnect_from()

        port = kwargs.get('port', TELNET_PORT)
        ...
        try:
            ...
            self._sock.settimeout(0.1)  # non-blocking reads in reader thread
            self._running = True

            # Wait briefly for GRBL welcome message
            time.sleep(1.5)
            try:
                startup = self._sock.recv(1024)
            ...
            self._reader_thread = threading.Thread(
                target=self._reader_loop, daemon=True, name="wgrbl-reader")
            self._reader_thread.start()

            # Start status polling thread
            self._status_thread = threading.Thread(
                target=self._status_poll_loop, daemon=True, name="wgrbl-status")
            self._status_thread.start()

            self.state = ConnectionState.CONNECTED
            device_info = {
                'device_name': f"WiFi GRBL ({address})",
                'ip': address,
                'port': port,
                'connection_type': 'wifi_grbl',
            }
            ...
            self.error_occurred.emit(f"WiFi GRBL connect failed: {exc}")
    ...
    def _write_bytes(self, data: bytes):
        """Write raw bytes to the TCP socket."""
        if self._sock is not None:
            try:
                self._sock.sendall(data)
            except Exception as exc:
                log.warning("WiFi GRBL write error: %s", exc)
                self.error_occurred.emit(f"Write error: {exc}")

    def _send_line(self, line: str, wait_ok: bool = True) -> str:
        """Send a line to GRBL over TCP."""
    ...
    def _send_realtime(self, byte_val: int):
        """Send a single-byte GRBL real-time command."""
        self._write_bytes(bytes([byte_val]))

    # ----- Reader thread -----

    def _reader_loop(self):
        """Background thread: read data from TCP socket."""
        line_buf = b''
        while self._running:
            ...
            try:
                ...
                if self._running:
                    self.error_occurred.emit("WiFi GRBL connection lost")
                    self._running = False
                    self.state = ConnectionState.DISCONNECTED
                    self.disconnected.emit()
                break
            ...
            if not chunk:
                ...
                if self._running:
                    self.error_occurred.emit("WiFi GRBL connection closed by remote")
                    self._running = False
                    self.state = ConnectionState.DISCONNECTED
                    self.disconnected.emit()
                break
    ...
    def _parse_grbl_status(self, raw: str):
        """Parse GRBL status report: <State|MPos:x,y,z|...>"""
        ...
        if am:
            acc = am.group(1)
            if 'S' in acc:
                status['spindle_state'] = 1
            elif 'C' in acc:
                status['spindle_state'] = 2
            coolant = 0
            if 'F' in acc:
                coolant |= 0x01
            if 'M' in acc:
    ...
    def _status_poll_loop(self):
        """Background thread: send '?' periodically for status reports."""
        while self._running:
            if self._sock is not None:
    ...
    def set_io(self, spindle_state: int = 0, spindle_rpm: int = 0,
        ...
        if coolant_state == 0:
            ...
            if coolant_state & 0x01:
                self._send_line('M8', wait_ok=False)
            if coolant_state & 0x02:
                self._send_line('M7', wait_ok=False)
